...
ERP <- filter(ERP, amplitude.mean <= 30 & amplitude.mean >= -30)
...
gavglme <- function(TP1, TP2, where) {
  ...
  lme_data <- filter(lme_data, region == where) # remove this line and change model to avg.mean ~ condition * region for full regions interactiions if interested. Not particularly informative here. 
...
# Some of the following pictures might not appear to be exactly like those in the thesis. This is because Z axis scales were manually adjusted in the function before producing each of the following pictures. Fixed scales for all or automatically adjustable scales did not yield particularly good results. If you check scale values from the thesis and substitute them before producing each picture in the above function, you will get matching results. 
